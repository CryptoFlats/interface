...
export const RaritySlider = () => {
  ...
  const classes = useMemo(
    () => [styles.pagination, window.innerWidth > 1200 ? "" : "container"],
    []
  );
...
const Background = React.memo<BackgroundProps>(({ activeIndex }) => {
  return (
...
const Slide = React.memo<SliderProps>(({ type, isActive, className }) => {
  const { t } = useTranslation();
  ...
  const content = useMemo(() => {
    const data: Record<SlideTypes, string[]> = {
      ...
      standardFlats: [
        ...
        "Daily Net Yield (Staking): 30-35 $CFLATS",
      ...
      silverFlats: [
        ...
        "Daily Net Yield (Staking): 36-40 $CFLATS",
        "Staking Duration: 180 days",
        "Unstake Available: After 5 days",
        "Reward Claim: 100% direct to wallet",
      ],
      goldFlats: [
        ...
        "Daily Net Yield (Staking): 41-45 $CFLATS",
        "Staking Duration: 180 days",
        "Unstake Available: After 5 days",
        "Reward Claim: 100% direct to wallet",
      ],
      diamondFlats: [
        ...
        "Daily Net Yield (Staking): 46-50 $CFLATS",
        "Staking Duration: 180 days",
        "Unstake Available: After 5 days",
        "Reward Claim: 100% direct to wallet",
      ],
  ...
  const roomClasses = useMemo(
    () => [isActive ? styles["is-active"] : "", styles.room].join(" "),
    [isActive]
  );

  const cardClasses = useMemo(
    () => [isActive ? styles["is-active"] : "", styles.card].join(" "),
    [isActive]
  );
    ...
    <div className={styles.wrapper}>
      <div className={styles.content}>
        ...
        <div className={cardClasses}>
          <div
            className={[styles["card-text"], styles[className || ""]].join(" ")}
          >
